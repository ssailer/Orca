  ...
  switch (state->last_tag) {
    ...
    case FCIOConfig: {
      ...
      if (processor->aux.tracemap_format) {
        int success = 0;
        success += convert_trace_idx(1, &processor->aux.pulser_trace_index, processor->aux.tracemap_format,
                                     state->config->tracemap);
        success += convert_trace_idx(1, &processor->aux.baseline_trace_index, processor->aux.tracemap_format,
                                     state->config->tracemap);
        success += convert_trace_idx(1, &processor->aux.muon_trace_index, processor->aux.tracemap_format,
                                     state->config->tracemap);
        if (processor->loglevel >= 4) {
        ...
        if (success != 3) {
          fprintf(
              stderr,
              "CRITICAL lpp_process_fcio_state: during conversion of aux channels, one channel could not be mapped.\n");
          return -1;
        }
      }

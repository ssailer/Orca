...
int LPPSetParametersFromFile(PostProcessor *processor, const char *setup_path) {
  ...
  if (!LPPSetGeParameters(processor, ge_nchannels, ge_channelmap, channelmap_format, majority_threshold,
                          skip_full_counting, ge_prescaling_threshold_adc, ge_average_prescaling_rate_hz)) {
    fprintf(stderr, "LPPSetGeParameters failed.");
    return 0;
  }

  if (!LPPSetSiPMParameters(processor, sipm_nchannels, sipm_channelmap, channelmap_format, calibration_pe_adc,
